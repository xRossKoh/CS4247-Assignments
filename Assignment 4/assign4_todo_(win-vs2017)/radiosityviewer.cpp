...
static const char radiosityModelFilename[] = "cornell_box_my.out";
...
int main(int argc, char** argv)
{
    ...
    winWidth = glutGet(GLUT_WINDOW_WIDTH);    // Required for macOS.
    winHeight = glutGet(GLUT_WINDOW_HEIGHT);  // Required for macOS.
    printf("Initial Window Size = %d x %d\n", winWidth, winHeight);

    MyInit();

    // Read model file.
    printf("Reading radiosity solution model from file %s...\n", radiosityModelFilename);
    model = RAD_ReadFile(radiosityModelFilename);

    // Make OpenGL display lists.
    printf("Making OpenGL display lists for model...\n");
