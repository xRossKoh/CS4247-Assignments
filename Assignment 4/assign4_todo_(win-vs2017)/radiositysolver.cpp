...
// Note that numPixelsOnWidth must be a even number.
{
    ...
    **********************************************************/
    ...
    for (int py = 0; py < numPixelsOnWidth / 2; py++)
    {
        double y = (py + 0.5) * dp;
        ...
        {
            double x = -1.0 + (px + 0.5) * dp;
            double dFq = (y * dA) / (M_PI * Sqr(x * x + y * y + 1.0));
            deltaFormFactors[py * numPixelsOnWidth + px] = (float)dFq;
        }
    }
}
...
// Need to set up the viewport, projection and view transfromation.
{
    ...
    **********************************************************/
    ...
    float lookDir[3], lookAt[3];

    VecDiff(lookDir, shooterQuad->v[face % 4], shooterQuad->v[(face - 1) % 4]);
    VecSum(lookAt, shooterQuad->centroid, lookDir);
