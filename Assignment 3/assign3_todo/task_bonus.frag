...
// COMMENTS TO GRADER: 
// Soft shadows implemented by modelling light sources as 
// 1 by 1 squares in the x-z plane, centered at Light.position.
// For each call of the CastRay function, each light source has
// its position offset within the aforementioned 1 by 1 square
// area, and the new offset light position is used in the ray
                      ...
                      out float t, out vec3 hitPos, out vec3 hitNormal )
{
    ...
    // Calculation assumes sphere is centered at origin
    // Translate ray origin to "sphere space", denoted by sc
    vec3 scRo = ray.o - sph.center;

    // a = 1.0, hence, not necessary to include in calculations
    float b = 2.0 * dot(ray.d, scRo);
    float c = dot(scRo, scRo) - sph.radius * sph.radius;

    // Finding discriminant
    float d = b * b - 4.0 * c;

    if (d < 0.0) // no solution, hence, no intersection
    {
        return false;
    }

    float t0 = -b / 2.0; // solution for d == 0.0
    if (d > 0.0)
    {
        float t1 = (-b - sqrt(d)) / 2.0; // t-
        float t2 = (-b + sqrt(d)) / 2.0; // t+
            
        if (t1 >= tmin && t1 <= tmax) t0 = t1;
        else if (t2 >= tmin && t2 <= tmax) t0 = t2;
        else return false;
    } else if (t0 < tmin || t0 > tmax) return false;

    vec3 hitPos0 = scRo + t0 * ray.d;
    vec3 hitNormal0 = normalize(hitPos0);

    t = t0;
    hitPos = hitPos0 + sph.center;
              ...
              out vec3 hitNormal, out vec3 k_rg )
{
    ...
    // Checking intersections with spheres
    ...
    // Ray_t object for shadow ray
    Ray_t shadowRay;
    shadowRay.o = nearest_hitPos;

    // View vector for Phong illumination calculation
    vec3 viewVec = normalize(ray.o - nearest_hitPos);

    // Accumulating lighting from each light source
    for (int i = 0; i < NUM_LIGHTS; i++)
    {
        // Light modelled as a 1 by 1 sqaure around Light.position in x-z plane 
        // Generating a random offset for light position
        vec3 offsetLightPos = Light[i].position + 
                                vec3(rand() - 0.5, 0.0, rand() - 0.5);
        vec3 shadowRayVec = normalize(offsetLightPos - nearest_hitPos);
        
        float tmax = distance(offsetLightPos, nearest_hitPos);
        tmax = min(tmax, DEFAULT_TMAX);
        
        shadowRay.d = shadowRayVec;

        bool inShadow = false;
        
        // Checking for shadow ray intersection with planes
        ...
        // Checking for shadow ray intersection with spheres
        ...
        I_local += PhongLighting(shadowRayVec, nearest_hitNormal, viewVec, 
                                inShadow, Material[nearest_hitMatID], Light[i]);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for (int i = 0; i < SPP; i++) {
        // Generate random offset for fragCoord for anti aliasing
        vec2 randomizedFragCoord = vec2(rand() - 0.5, rand() - 0.5) + fragCoord;

        // Current pixel 3D position in camera space.
        vec3 pixel_pos = image_origin +
                        vec3(pixel_width * randomizedFragCoord, 0);

        // Create primary ray.
        Ray_t pRay;

        // Generate random offset for cam pos for DOF effect
    ...
    fragColor = vec4( sumColor / float(SPP) , 1.0 );
}
