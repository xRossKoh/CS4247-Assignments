...
void InitScene()
{
    ...
    // Black static sphere.
    Sphere[0].center = vec3( 1.0 , 0.7, -0.5 );
    Sphere[0].radius = 0.7;
    Sphere[0].materialID = 3;

    // Green circling sphere.
    Sphere[1].center = vec3( 3.0 * cos(iTime) , 0.5, 3.0 * sin(iTime) );
    Sphere[1].radius = 0.5;
    Sphere[1].materialID = 2;
    
    // Gold bouncing sphere.
    Sphere[2].center = vec3( 0.0 , abs(sin( 3.0 * iTime)) + 0.2, 0.0 );
    Sphere[2].radius = 0.2;
    ...
    // Light 0.
    Light[0].position = vec3( 5.0 * cos(iTime), 6.0, 1.0 * sin(iTime) );
    Light[0].I_a = vec3( 0.1, 0.1, 0.1 );
    Light[0].I_source = vec3( 0.5, 0.3, 0.3 );

    // Light 1.
    Light[1].position = vec3( 2.0 * cos(iTime), 2.0 * sin(iTime) + 7.0 , 1.5 );
    Light[1].I_a = vec3( 0.1, 0.1, 0.1 );
    Light[1].I_source = vec3( 0.8, 0.5, 0.3 );
}
                      ...
                      in float tmin, in float tmax )
{
    ...
    // Falculation assumes sphere is centered at origin
    // Translate ray origin to "sphere space", denoted by sc
    vec3 scRo = ray.o - sph.center;
              ...
              out vec3 hitNormal, out vec3 k_rg )
{
    ...
    {
        vec3 shadowRayVec = normalize(Light[i].position - nearest_hitPos);
        ...
        // Check for shadow ray intersection with planes
        ...
        // Check for shadow ray intersection with spheres
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 cam_pos = vec3( 0.0, 3.0, 6.0 );
    vec3 cam_lookat = vec3( 0.25, 1.0, 0.0 );
    vec3 cam_up_vec = vec3( 0.0, 1.0, -0.3 );
    ...
    const float aperture_width = 0.2;
    //=======================================================================
